...
U32 MatMulDMA_Get_Status(void)
{
    return RegRead(MATMUL_DMA_STATUS_ADDR);
}

U32 MatMulDMA_Get_CRC32(void)
{
    return RegRead(MATMUL_DMA_CRC32_ADDR);
}

int MatMulDMA_Start(U32 src_phys, U32 dst_phys, U32 groups)
{
    U32 status = MatMulDMA_Get_Status();
    ...
    RegWrite(MATMUL_DMA_STATUS_ADDR,
             MATMUL_DMA_STATUS_DONE | MATMUL_DMA_STATUS_ERROR);
...
int MatMulDMA_Wait(U32 timeout_polls)
{
    U32 polls = 0u;
    U32 status;

    for (;;) {
        status = MatMulDMA_Get_Status();
